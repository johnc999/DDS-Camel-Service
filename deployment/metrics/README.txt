Startup
java -agentlib:jdwp=transport=dt_socket,server=n,suspend=y,address=localhost:64820 -javaagent:./target/jmx_prometheus_javaagent.jar=9779:./configuration/Metrics/prometheus-agent-config.yml -jar ./target/Integration-Platform-0.0.1-SNAPSHOT.jar --spring.config.location=./src/main/resources/application.yml --spring.config.location=./configuration/dds.properties --spring.config.location=./configuration/mq.properties
...
Example Searches:
  Label and free text in single search
     {service="dds-camel-service"} |= "Start"
